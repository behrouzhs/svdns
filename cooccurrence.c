...
#define WEIGHT_COOCCUR(distance_weighting, j, k) distance_weighting == 0 ? 1 : distance_weighting == 1 ? 1.0/((real)(j-k)) : distance_weighting == 2 ? 2.0/((real)(1.0+j-k)) : distance_weighting == 3 ? 3.0/((real)(2.0+j-k)) : 1.0/sqrt((real)(j-k))
...
real memory_limit = 96; // soft limit, in gigabytes, used to estimate optimal array sizes
...
char output_filename[512] = "cooccurrence.bin";
...
int merge_files(int num) {
	...
	if (num == 1) {
		...
		while (1) {
			temp_fread_out = fread(&temp_record, sizeof(CREC), 1, fid);
			if (feof(fid) || temp_fread_out == 0) {
				fclose(fid);
				fclose(fout);
				remove(filename);
				fprintf(stderr,"\n");
			...
			fwrite(&temp_record, sizeof(CREC), 1, fout);
		}
	}
    ...
    for (i = 0; i < num; i++) {
		...
		temp_fread_out = fread(&new, sizeof(CREC), 1, fid[i]);
        new.id = i;
        insert(pq,new,i+1);
    }
    ...
    while (1) {
        ...
        for (k = j - 1; k >= ( (j > window_size) ? j - window_size : 0 ); k--) { // Iterate over all words to the left of target word, but not past beginning of line
            ...
            if ( w1 < max_product/w2 ) { // Product is small enough to store in a full array
                bigram_table[lookup[w1-1] + w2 - 2] += WEIGHT_COOCCUR(distance_weighting, j, k); // Weight cooccurrence according to the distance
                if (symmetric > 0) bigram_table[lookup[w2-1] + w1 - 2] += WEIGHT_COOCCUR(distance_weighting, j, k); // If symmetric context is used, exchange roles of w2 and w1 (ie look at right context too)
            }
                ...
                cr[ind].word1 = w1;
                cr[ind].word2 = w2;
                cr[ind].val = WEIGHT_COOCCUR(distance_weighting, j, k);
...
void print_usage() {
	...
	printf("\nExample usage:\n");
	printf("./cooccur -verbose 2 -symmetric 0 -window-size 10 -vocab-file vocab.txt -memory 8.0 -overflow-file tempoverflow -input corpus.txt -output cooccurrences.bin\n\n");
}
...
int main(int argc, char **argv) {
	...
	//fprintf(stderr, "rlimit: %f\r\n", rlimit);
	//fprintf(stderr, "memory_limit: %f\r\n", memory_limit);
	//fprintf(stderr, "max_product: %lld\r\n", max_product);
	//fprintf(stderr, "overflow_length: %lld\r\n", overflow_length);
